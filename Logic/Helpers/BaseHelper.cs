...
namespace Logic.Helpers
{
	...
	public class BaseHelper : IBaseHelper
	{
		...
		public BaseHelper(AppDbContext context, ICacheService cacheService)
		{
			_context = context;
		...
		public int GetCompanyPageSize(Guid? companyId)
		{
			var pageSize = GetByPredicate<Notification>(x => x.CompanyId == companyId).FirstOrDefault();
			return pageSize != null && pageSize.PageSizeData.HasValue ? pageSize.PageSizeData.Value : 25;
		}
		public int PageSizeFromRedisCache(Guid? currentBranchId)
		{
			return _cacheService.GetFromCacheAddIfNotFound(RedisKeyHelper.PageSize(currentBranchId),
			() => GetCompanyPageSize(currentBranchId),
